...
| 日期 | 版本 | 更新内容 | 更新人 |
|------|------|----------|--------|
| 2026-08-10 | v2.6 | 新增 CLOUD-P0-15 回收订单管理 + CLOUD-P0-16 商城订单管理（11字段列表+状态说明+详情页+筛选搜索+业务规则+验收标准） | 何子豪 |
| 2026-08-10 | v2.5 | 补充账号激活流程：明确6角色激活范围(OWN/CLK/WH走激活，OPS/DB/ENT/SA直接active) + 未激活清理规则 + 已有账号免激活 + removed状态详细说明 | 何子豪 |
| 2026-08-10 | v2.4 | 平板绑定策略调整：平板仍绑定门店(资产管理)，但店员登录不校验门店匹配 | 何子豪 |
| 2026-08-10 | v2.3 | 补充审核界面要素：审核列表字段定义(9字段+6筛选+排序规则) + 设备状态说明(3状态+标签颜色) + 列表交互规则 | 何子豪 |
| 2026-08-10 | v2.2 | 运营审核流程重构：取消手动调整成色等级，运营仅补勾扣款项，系统自动重新判定等级（消除等级与扣款项不一致的逻辑缺陷） | 何子豪 |
| 2026-08-10 | v2.1 | OTP 有效期从 60 秒延长至 3 分钟（冷却/锁定/重发保持 60 秒不变） | 何子豪 |
...
| CLOUD-P0-15 | 回收订单管理 | P0 |
| CLOUD-P0-16 | 商城订单管理 | P0 |
...
**优先级：** P0
**功能描述：**
运营人员在 Web 后台对待上架设备进行审核。审核时对比质检影像与系统判定的扣款项是否一致。若一致则直接上架；若不一致，运营人员在审核界面补勾店员遗漏的扣款项（外观缺陷/硬件扣款），提交后系统自动重新判定成色等级并计算回收价和商城售价，然后上架。不设"拒绝上架"操作——所有设备均会上架。**运营不手动调整成色等级，等级完全由系统根据扣款项通过逐级熔断规则自动判定。**
**审核流程：**
```
待上架设备进入任务队列
  → 运营人员打开审核工作台
...
| 商品主图上传 | 运营从系统图片库选择（图片库由运营在 CLOUD-P2-03 设备品类管理中按品牌/型号预先上传的官方精修产品图）或本地上传一张新机精修图作为商城商品列表主图（与质检原始照片区分），支持预览裁剪 |
| 操作区 | "直接上架"按钮（机况一致）+ "调整后上架"按钮（机况不符，展开调整面板） |
**审核任务队列列表字段：**
| 字段 | 说明 | 数据来源 |
|------|------|----------|
| 设备标识 | 品牌 + 型号 + IMEI 后四位（格式：`iPhone 12 · IMEI ****1234`） | 质检数据 |
| 存储/颜色 | 存储容量 + 颜色（格式：`128GB · 黑色`） | 质检数据 |
| 来源门店 | 回收该设备的门店名称 + 门店编码 | 到店会话关联 |
| 店员 | 执行质检的店员编号 + 姓名 | 平板登录账号 |
| 入库时间 | 仓库确认入库的时间（格式：`YYYY-MM-DD HH:mm`） | 入库确认时间戳 |
| 当前成色等级 | 系统自动判定的成色等级标签（A/B/C/D 彩色标签） | 定价引擎 |
| 系统回收价 | 定价引擎计算的初始回收价（INR） | 定价引擎 |
| 待审时长 | 从入库到当前的等待时长，超 24 小时橙色标记 | 实时计算 |
| 操作 | "开始审核"按钮 → 打开审核工作台 | — |
**审核任务队列筛选条件：**
| 筛选项 | 类型 | 说明 |
|--------|------|------|
| 品牌 | 下拉单选 | 按品牌筛选，选项动态加载已配置品牌 |
| 机型 | 下拉单选（级联品牌） | 按机型筛选，选择品牌后加载对应型号 |
| 成色等级 | 多选 A/B/C/D | 按系统当前判定的等级筛选 |
| 来源门店 | 下拉多选 | 按回收门店筛选 |
| 入库时间 | 日期范围 | 支持快捷选择（今日/昨日/本周/本月/自定义） |
| 待审时长 | 单选 | 全部 / 24小时内 / 已超时（>24h） |
**审核任务队列排序：**
| 默认排序 | 说明 |
|----------|------|
| 入库时间升序 | 先入库的先审核（FIFO），保证公平性 |
| 超时优先 | 超 24 小时未审的设备自动置顶（橙色高亮） |
**列表设备状态说明：**
审核任务队列中出现的设备状态及其含义：
| 状态 | 英文标识 | 含义 | 标签颜色 |
|------|----------|------|:---:|
| 待上架申请 | `pending_review` | 用户已接受报价+核销完成，仓库已确认入库，等待运营审核上架 | 蓝色 |
| 审核超时 | `pending_review` (overdue) | 入库后超过 24 小时未审核，标记橙色提醒，仍在待审核队列中 | 橙色 |
| 在库可售 | `available_for_sale` | 审核已完成并上架，可在商城被浏览和购买（审核历史中可见，不在待审队列中） | 绿色 |
**审核任务队列交互：**
| 交互 | 说明 |
|------|------|
| 点击行 | 点击任意设备行 → 打开该设备的审核工作台（右侧滑出面板或新页面） |
| 批量操作 | 不支持批量审核（每台设备须逐台查看影像后单独审核） |
| 快捷键 | `←` `→` 键切换上一台/下一台设备审核工作台 |
| 离开确认 | 若已展开调整面板但未提交，切换设备时弹出"确认离开？调整内容将不会保存" |
**运营调整面板（点击"调整后上架"时展开）：**
| 可调整项 | 说明 |
|----------|------|
| 外观扣款项 | 可新增/修改店员未勾选的扣款项（如店员遗漏了屏幕划痕，运营可补勾 CO-SCR-02 扣款项） |
| 硬件扣款项 | 可新增店员漏判的硬件扣款（如店员未标记触控异常，运营从检测结果中确认后补勾） |
| 调整原因 | 必填，文本输入框，简要说明为何调整（如"屏幕划痕在视频中清晰可见，店员未勾选"），最少 10 字 |
| 成色等级预览 | 系统根据调整后的扣款项实时预览重新判定后的成色等级（仅供预览，运营不可手动修改） |
**调整后系统自动执行：**
| 动作 | 说明 |
|------|------|
| 重新判定成色等级 | 基于调整后的扣款项，重新跑逐级熔断逻辑（CLOUD-P0-01 §3.3.2.2）输出新等级 |
| 重新计算回收价 | 基于新的成色等级和新增扣款项，重新跑定价引擎（CLOUD-P0-01） |
| 重新计算商城售价 | 基于新的回收价和成色等级，重新计算挂牌售价 |
| 推送调整详情至平板 | 通知门店：调整了哪些扣款项、等级变化（如有）、原价格 → 调整后价格、调整原因（参见 TAB-P0-06） |
| 设备状态 → [在库可售] | 直接上架至商城，不阻塞流转 |
**业务规则：**
| 规则项 | 说明 |
|--------|------|
| 无拒绝操作 | 所有设备均上架，不存在"拒绝上架"按钮；极端劣质设备可通过补勾多项扣款压低价格后上架 |
| 等级由系统判定 | 运营不手动调整成色等级；等级由定价引擎根据扣款项通过逐级熔断规则自动重新判定，确保等级与扣款项始终一致 |
| 调整原因必填 | 任何时候选择"调整后上架"，调整原因为必填字段，不少于 10 个字符 |
| 商品主图必填 | 无论直接上架还是调整后上架，运营必须上传至少一张商品主图，未上传不可提交上架 |
| 调整可叠加 | 运营可在同一审核会话中同时补勾多项外观扣款和硬件扣款 |
| 不可回退 | 上架操作一旦提交不可撤回，设备立即进入在库可售状态 |
| 审核时效 | 设备入库后超过 24 小时未审核，系统在任务队列中标记"超时待审"橙色提醒 |
| 调整日志 | 所有调整操作记录操作人、时间、调整前后对比（扣款项变化 + 等级变化 + 价格变化）、调整原因，用于门店复盘和运营绩效考核 |
**验收标准：**
...
- [ ] 机况不符时，运营点击"调整后上架"展开调整面板：可补勾/新增扣款项、实时预览系统重新判定的成色等级（不可手动修改等级）、必须填写调整原因（≥10 字）
...
| ROLE-WH（库管） | SA 或 OWN | ✅ 是 | 创建后发送邀请 SMS → 库管首次登录强制激活 |
| ROLE-OPS（运营人员） | SA | ❌ 否 | 内部员工，SA 创建后直接设为 active（无需激活，SA 口头通知） |
...
> **规则：** 需要绑定门店/仓库的外部角色（OWN/CLK/WH）走激活流程；不绑定组织的内部角色（OPS/DB/ENT/SA）直接激活。
**特殊情况——已有平台账号：**
...
| 平板绑定 | 平板设备绑定唯一门店，用于资产管理和远程管控（如门店关店时远程锁定平板）；店员登录**不校验**门店匹配，店员可在任意已注册平板上登录 |
### 业务规则
| 规则项 | 说明 |
|--------|------|
...
**优先级：** P0
**功能描述：**
运营人员在后台查看全平台所有回收订单，支持按门店、状态、时间范围等维度筛选和搜索。回收订单记录从用户到店质检到核销完成的完整链路，是平台的**核心业务流水**。运营通过此模块进行客服排查、异常处理和数据核对。
**回收订单列表字段：**
| 字段 | 说明 | 示例 |
|------|------|------|
...
| 质检等级 | 系统判定 / 运营调整后的最终等级 | A / B / C / D（彩色标签） |
...
| 规则项 | 说明 |
|--------|------|
| 数据范围 | OPS 可查看全平台回收订单；OWN 仅可查看本门店回收订单 |
| 只读 | 回收订单为历史记录，运营不可修改任何字段（仅查看） |
...
- [ ] 详情页操作日志时间线展示全链路节点完整
- [ ] 筛选结果支持导出 Excel/CSV
- [ ] OWN 角色仅可查看本门店回收订单，OPS 可查看全平台
- [ ] 已核销订单与审核记录、商城设备正确关联

## CLOUD-P0-16 商城订单管理
**优先级：** P0
**功能描述：**
运营人员在后台查看全平台所有商城订单（B2C 和 B2B），支持按订单类型、支付状态、发货状态、时间范围等维度筛选。商城订单是平台的**核心收入流水**，运营通过此模块进行订单查询、异常处理（取消订单/退款）、客服支持和财务对账。
**商城订单列表字段：**
| 字段 | 说明 | 示例 |
|------|------|------|
...
| 已取消 | `cancelled` | 订单已取消（超时/用户主动/运营取消） | — | 灰色 |
...
| 规则项 | 说明 |
|--------|------|
| 数据范围 | OPS 可查看全平台商城订单；OWN 仅可查看本门店 B2B 采购订单 |
...
- [ ] OWN 角色仅可查看本门店 B2B 采购订单，OPS 可查看全平台
- [ ] 筛选结果支持导出 Excel/CSV
