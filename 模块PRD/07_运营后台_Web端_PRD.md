...
| 日期 | 版本 | 更新内容 | 更新人 |
|------|------|----------|--------|
| 2026-08-11 | v2.2 | 移除平板绑定门店逻辑（平板安装即用无需注册）；编码体系/权限矩阵去掉平板设备编码和device:admin权限；门店关店去掉平板远程锁定 | 何子豪 |
| 2026-08-10 | v2.1 | OTP有效期60秒→3分钟；运营审核重构(取消手动调等级/补审核列表)；平板绑定策略调整(登录不校验门店)；账号激活流程补充(角色范围+清理规则)；removed状态详细说明；新增回收订单管理+商城订单管理 | 何子豪 |
...
| 门店关店 | 状态 active → closed；触发关联账号批量禁用（详见门店App PRD 账号生命周期） |
