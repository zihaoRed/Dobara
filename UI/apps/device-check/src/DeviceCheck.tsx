...
import { getH5SessionState, H5ApiError, H5SessionState, openH5Session } from './h5Api';

/**
 * TAB-P0-14 — on-device H5 check page (runs in the browser of the phone being inspected).
 * Opened by the store phone (Android: ADB VIEW intent / iOS: QR scan) with a one-time token.
 * Opening the page exchanges that token with the H5 backend, then polls remote state every 2s.
 * Physical buttons are guided here but judged by the store phone via ADB — not by this page.
 */
...
const H5_ITEM_TO_STEP: Partial<Record<string, TStepKey>> = {
  screen_display: 'screen', touch: 'touch', sensor: 'sensors', speaker: 'speaker',
  microphone: 'mic', camera: 'camera', button: 'buttons',
};

function resultFromItemStatus(status: number): TResult {
...
export default function DeviceCheck() {
    ...
    if (next.sessionStatus === 2) {
      setResults((previous) => {
        ...
        next.items.forEach((item) => {
          const key = H5_ITEM_TO_STEP[item.itemCode];
          if (key) merged[key] = resultFromItemStatus(item.itemStatus);
        });
        return merged;
      });
      setPhase('done');
  ...
  const beginSession = async (candidate = token) => {
    ...
    setSessionError('');
    try {
      const opened = await openH5Session(detectToken, OPEN_IDEMPOTENCY_KEY);
      applySession(opened);
      if (opened.sessionStatus !== 2) setPhase('testing');
    } catch (error) {
  ...
  // A URL issued by the store phone counts as opening the detection page: no extra tap required.
  useEffect(() => {
    if (URL_TOKEN) void beginSession(URL_TOKEN);
    // OPEN_IDEMPOTENCY_KEY makes React StrictMode's development re-run safe.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (phase === 'token' || !session || session.sessionStatus !== 1) return;
    let stopped = false;
  ...
  }, [phase, session?.sessionId, session?.sessionStatus, token]);
  ...
  return (
    <div className="dc-app" style={{ display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto', width: '100%' }}>
      ...
      {phase === 'testing' && step && (
        <>
          <Header stepIdx={stepIdx} token={token} session={session} error={sessionError} />
      ...
      {phase === 'done' && <Summary token={token} results={results} session={session} error={sessionError} />}
    </div>
  );
}
...
function Header({ stepIdx, token, session, error }: { stepIdx: number; token: string; session: H5SessionState | null; error: string }) {
  return (
    <header className="dc-header" style={{ background: BRAND, color: '#fff', padding: '14px 16px' }}>
      ...
      <p style={{ fontSize: 12, color: error ? '#ffd3cf' : '#d7eee6', marginTop: 4 }}>
        {error || (session ? `Server session ${session.sessionId} · status ${session.sessionStatus}` : 'Opening server session…')}
      </p>
    </header>
  );
}
...
function TokenGate({ token, setToken, linking, error, onStart }: {
  token: string; setToken: (t: string) => void; linking: boolean; error: string; onStart: () => void;
}) {
  return (
...
function SensorsTest({ onDone }: { onDone: (r: TResult) => void }) {
  ...
  const [support, setSupport] = useState<'checking' | 'listening' | 'insecure' | 'unsupported'>('checking');
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {support === 'unsupported' && (
        <p style={{ fontSize: 13, color: '#92610a', textAlign: 'center', lineHeight: 1.5 }}>
          No orientation events were received. Check whether this browser exposes motion sensors and whether motion permission is enabled.
        </p>
      )}
      {support === 'insecure' && (
        <p style={{ fontSize: 13, color: '#92610a', textAlign: 'center', lineHeight: 1.5 }}>
          Local HTTP blocks real motion-sensor access. This is a browser security restriction, not a hardware failure. Use the demo simulation below or test from a trusted HTTPS origin.
        </p>
      )}
      ...
      <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {IS_DEMO && !allDone && (
          <button style={btn('ghost')} data-testid="dc-sensors-simulate" onClick={simulateMotion}>
            Simulate this axis · local demo
          </button>
        )}
        {(support === 'unsupported' || support === 'insecure') && (
...
function Summary({ token, results, session, error }: {
  token: string; results: Record<TStepKey, TResult>; session: H5SessionState | null; error: string;
}) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 24, gap: 16 }}>
      <div style={{ textAlign: 'center', marginTop: 24 }}>
        ...
        <p style={{ fontSize: 13, color: error ? DANGER : session?.sessionStatus === 2 ? GREEN : '#92610a', marginTop: 6 }}>
          {error || (session?.sessionStatus === 2 ? 'Server session completed ✓' : 'Waiting for server session completion…')}
        </p>
      </div>
